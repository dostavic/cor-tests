...
TEST(main_t05_trajectory, parseArguments){
    Arguments arguments;
    int argc = 4;
    char *argv[] = {
        (char*)"./InfilGenerator",
        (char*)"./tst/InfilGenerator/correctInput/trajectory.dat0_T05",
        (char*)"./tst/InfilGenerator/outfill/trajectry_out.dat0_T05",
        (char*)"trajectory"
    };
    ...
    EXPECT_EQ(arguments.inputFile, 
        "./tst/InfilGenerator/correctInput/trajectory.dat0_T05");
    EXPECT_EQ(arguments.filenamePrefix, 
        "./tst/InfilGenerator/outfill/trajectry_out.dat0_T05");
    EXPECT_EQ(arguments.infilKind, InfilKind::TRAJECTORY);
}

TEST(main_t05_trajectory, readInput){
    Arguments argumets;
    argumets.inputFile = "./tst/InfilGenerator/correctInput/trajectory.dat0_T05";
    ...
    EXPECT_NO_THROW(outfilData->readInput(&(argumets.inputFile)));
}

TEST(main_t05_trajectory, readInputImpl){
    std::unique_ptr<OutfilData> outfilData(new TrajectoryOutfilTest());
    TrajectoryOutfilTest *trajectoryOutfilTest = 
        dynamic_cast<TrajectoryOutfilTest*>(outfilData.get());
    char buffer[bufferSize];
    std::string inputFile = "./tst/InfilGenerator/correctInput/trajectory.dat0_T05";
...
TEST(main_t05_trajectory, readSecondLine){
    std::unique_ptr<OutfilData> outfilData(new TrajectoryOutfilTest());
    TrajectoryOutfilTest *trajectoryOutfilTest = 
        dynamic_cast<TrajectoryOutfilTest*>(outfilData.get());
    char buffer[bufferSize];
    std::string inputFile = 
        "./tst/InfilGenerator/correctInput/trajectory.dat0_T05_line_2";
    FILE *gg = fopen(inputFile.c_str(), "r");
    ...
    EXPECT_EQ(trajectoryOutfilTest->version, Version::T05);
...
TEST(main_t05_trajectory, readParmod){
    std::unique_ptr<OutfilData> outfilData(new TrajectoryOutfilTest());
    TrajectoryOutfilTest *trajectoryOutfilTest = 
        dynamic_cast<TrajectoryOutfilTest*>(outfilData.get());
...
TEST(main_t05_trajectory, readOMNI){
    std::unique_ptr<OutfilData> outfilData(new TrajectoryOutfilTest());
    TrajectoryOutfilTest *trajectoryOutfilTest = 
        dynamic_cast<TrajectoryOutfilTest*>(outfilData.get());
    TrajectoryOutfilTest *castData = 
    ...
    EXPECT_FLOAT_EQ(castData->tIndices.W2, 0.14);
    EXPECT_FLOAT_EQ(castData->tIndices.W3, 0.09);
    EXPECT_FLOAT_EQ(castData->tIndices.W4, 0.08);
    EXPECT_FLOAT_EQ(castData->tIndices.W5, 0.15);
    EXPECT_FLOAT_EQ(castData->tIndices.W6, 0.19);
    EXPECT_EQ(castData->tIndices.success, LineState::NOT_FOUND);
    EXPECT_EQ(omniError, 0);
}

TEST(main_t05_trajectory, writeData){
    Arguments arguments;
    ...
    FILE *f = fopen("./tst/InfilGenerator/outfill/trajectry_out.dat0_T05_0", "r");
    ...
    EXPECT_FLOAT_EQ(W1, 0.15);
    EXPECT_FLOAT_EQ(W2, 0.14);
    EXPECT_FLOAT_EQ(W3, 0.09);
    EXPECT_FLOAT_EQ(W4, 0.08);
    EXPECT_FLOAT_EQ(W5, 0.15);
    EXPECT_FLOAT_EQ(W6, 0.19);
